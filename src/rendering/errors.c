...
void gui_SetLog(uint8_t type, const char* msg){
    uint8_t i;
    for(i = 3; i > 0; i--){
        strncpy(&log_msg[i], &log_msg[i-1], 50);
        icon[i] = icon[i-1];
        log_display[i] = log_display[i-1];
    }
    memset(&log_msg[0], 0, 50);
    strncpy(&log_msg[0], msg, 50);
...
void gui_ShowLog(void){
    ...
    uint8_t y = 216-2;
    for(i=0; i<4; i++){
        if(!log_display[i]) memset(&log_msg[i], 0, 50);
        if(*log_msg[i]) h+=10;
    }
    h+=6;
    ...
    for(i=0; i<4;i++){
        if(*log_msg[i]){
            ...
            gfx_PrintStringXY(log_msg[i], x+12, y+4);
            y-=10;
        }
    }
}
