...
int MOCKABLE pico_ipv4_route_add(const struct pico_ip4 *address, const struct pico_ip4 *netmask, const struct pico_ip4 *gateway, int metric, struct pico_ipv4_link *link)
{
...
int pico_ipv4_link_add(struct pico_device *dev, const struct pico_ip4 *address, const struct pico_ip4 *netmask)
{
    ...
    gateway.addr = 0U;
    int err = pico_ipv4_route_add(&network, netmask, &gateway, 1, new);
    if(err) {
        dbg("error adding route: %i\n", err);
        return -1;
    }
