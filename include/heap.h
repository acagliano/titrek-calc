...
#define DECLARE_HEAP(type, orderby) \
    ...
    static inline type* heap_get_element(struct heap_ ## type *heap, uint24_t idx) \
    { \
        uint24_t elements_per_block = MAX_BLOCK_SIZE/sizeof(type); \
    ...
    {\
        type *newTop; \
        uint24_t elements_per_block = MAX_BLOCK_SIZE/sizeof(type); \
        uint24_t elements = (heap->n + 1)%elements_per_block;\
